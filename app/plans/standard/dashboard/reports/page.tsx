...
export default function ReportsPage() {
  ...
  const dropMenuBg = theme === "light" ? "bg-white border-gray-200" : "bg-[#1A1F2E] border-[#2A303C]";
  const dropMenuItem = theme === "light" ? "text-gray-700 hover:bg-gray-100" : "text-gray-300 hover:bg-[#2A303C]";
  const textSubCardContent = theme === "light" ? "text-gray-600" : "text-gray-300";
    ...
    <div className={`flex-1 flex flex-col p-6 overflow-y-auto transition-colors duration-300 ${bgMain} ${textMain}`}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className={`text-sm ${textSecondary}`}>Download detailed performance reports in PDF or CSV format.</p>
        </div>
      </div>

      {!canDownload && (
        <div className={`${warningBg} border rounded-lg p-4 mb-6 text-sm`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Monthly Report Card */}
        <div className={`${bgCard} border rounded-xl p-6 flex flex-col`}>
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className={`${textPrimary} font-medium text-lg`}>Monthly PDF Report</h3>
                <p className={`text-sm ${textSecondary}`}>Complete performance summary for the month</p>
              </div>
            </div>
            
            <div className={`${bgSubCard} border rounded-lg p-4 mb-4 text-center`}>
              <p className={`text-sm ${textSubCardContent} leading-relaxed`}>
            ...
            <div className="relative">
              ...
              {showMonthlyMenu && canDownload && (
                <div className={`absolute bottom-full left-0 w-full mb-2 ${dropMenuBg} border rounded-lg shadow-lg overflow-hidden z-10`}>
                  <button
                    onClick={() => handleDownloadMonthly('pdf')}
                    className={`w-full flex items-center gap-2 px-4 py-2 ${dropMenuItem} text-sm transition-colors`}
                  >
        ...
        <div className={`${bgCard} border rounded-xl p-6 flex flex-col`}>
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className={`${textPrimary} font-medium text-lg`}>Weekly Performance Report</h3>
                <p className={`text-sm ${textSecondary}`}>Track weekly performance trends</p>
              </div>
            </div>
        ...
        <div className={`md:col-span-2 ${bgCard} border rounded-xl p-6`}>
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className={`${textPrimary} font-medium text-lg`}>6 Months Data History</h3>
                <p className={`text-sm ${textSecondary}`}>Complete historical data for last 6 months</p>
              </div>
            </div>
          </div>
      ...
      {showHistoryModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className={`${modalBg} border rounded-xl p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto`}>
            <div className="flex justify-between items-center mb-6">
              <h2 className={`${textPrimary} font-bold text-xl`}>Last 6 Months History</h2>
              <button
            ...
            {historyLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {historyData.map((item, index) => (
                  <div key={index} className={`${bgSubCard} border rounded-lg p-4`}>
                    <div className="flex justify-between items-center">
                      <div>
                        <p className={`${textPrimary} font-medium`}>{item.month}</p>
